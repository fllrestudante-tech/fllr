...
test("nunca teve sucesso: score 0, state never_succeeded", () => {
  const result = computeFreshnessScore("funding", null);
  assert.equal(result.score, 0);
  assert.equal(result.state, "never_succeeded");
  assert.equal(result.ageMs, null);
});

test("exemplo do usuário: Funding SLA 5min, última coleta há 2min -> 100 (fresh)", () => {
  const now = new Date("2026-07-16T08:02:00.000Z").getTime();
  const result = computeFreshnessScore("funding", "2026-07-16T08:00:00.000Z", now);
...
test("exemplo do usuário: Funding SLA 5min, última coleta há 13h -> 0 (muito além da tolerância)", () => {
  const now = new Date("2026-07-16T13:00:00.000Z").getTime();
  const result = computeFreshnessScore("funding", "2026-07-16T00:00:00.000Z", now);
  assert.equal(result.score, 0);
  assert.equal(result.state, "stale");
});

test("zona de tolerância: decai linearmente entre expectedIntervalMs e expectedIntervalMs*toleranceMultiplier", () => {
  // funding: expected=5min, tolerance=2x -> decai de 5min(100) a 10min(0). Em 7min30s (metade do range de decaimento) -> ~50.
