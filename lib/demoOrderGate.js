...
const OPERATION_KIND = Object.freeze({
  ...
  SAFE_LEVERAGE_REDUCTION: "SAFE_LEVERAGE_REDUCTION",
  AMBIGUOUS: "AMBIGUOUS",
});

// tradeMode esperado pra qualquer operação demo -- 0 = Cross Margin,
// mesma convenção já usada em todo o resto do código (positionIdx=0
// pra one-way mode em placeOrder/setTradingStop) e confirmada como o
// valor real da conta na auditoria de leitura desta rodada.
const EXPECTED_TRADE_MODE = 0;
 ...
 *
 * setLeverage sempre classifica como SAFE_LEVERAGE_REDUCTION (item 4 da
 * Rodada 6) -- nunca INCREASE_EXPOSURE. A classificação em si continua
 * PURA/sem I/O (só rotula a chamada como "candidata a redução segura");
 * quem decide se é de fato uma redução legítima (leverage solicitada
 * estritamente menor que a efetiva, conta flat, sem ordens abertas,
 * modo de posição compatível) é assertDemoOrderAllowedInner, que lê o
 * snapshot confiável ANTES de decidir -- nunca este classificador.
 * Qualquer tentativa que NÃO seja uma redução válida (aumento, valor
 * igual, estado desconhecido) é bloqueada dentro dessa validação, nunca
 * silenciosamente promovida a um aumento de exposição armado.
 */
function classifyBybitOperation(opName, params = {}, positionSnapshot = null) {
  switch (opName) {
    ...
    case "setLeverage":
      return OPERATION_KIND.SAFE_LEVERAGE_REDUCTION;
    case "placeOrder":
...
function assertDemoOrderAllowedInner({ env, opName, params, now }) {
  ...
  if (kind === OPERATION_KIND.SAFE_LEVERAGE_REDUCTION) {
    // Redução segura de leverage (item 4 da Rodada 6) -- NUNCA exige
    // ARMED_DEMO (funciona inclusive em BLOCK_NEW_EXPOSURE/
    // EMERGENCY_EXIT_ONLY, o mesmo espírito dos defensiveKinds acima:
    // baixar leverage reduz risco, nunca aumenta), e NUNCA passa pelo
    // lock de reserva (não reserva capacidade financeira nenhuma). Ainda
    // assim, diferente dos defensiveKinds, exige um snapshot FRESCO e
    // íntegro -- só ele sabe a leverage efetiva real da conta, sem a
    // qual não há como provar que "10 -> 2" é de fato uma redução.
    return assertSafeLeverageReductionAllowed({ env, params, limits, snapshotPath, now });
  }
  ...
  return reservationLock.withReservationLock(lockPath, () => {
    ...
    const order = {
      ...
      side: params.side,
      orderType: params.orderType, // item 2 da Rodada 5 -- decide maxOrderQty (Limit) vs maxMktOrderQty (Market), nunca assumido
      qty: params.qty,
      price: params.price,
    ...
    // symbolState (leverage efetiva/tradeMode/positionIdx, item 4 da
    // Rodada 5) vem EXCLUSIVAMENTE do snapshot confiável -- `null` quando
    // o snapshot não conseguiu determinar um estado inequívoco (ver
    // lib/demoAccountSnapshot.js::extractSymbolState), o que faz
    // validateDemoOrder bloquear por "desconhecido", nunca presumir.
    const symbolState = authoritative.snapshot.symbolState;
    const riskState = {
      ...
      effectiveLeverage: symbolState ? symbolState.effectiveLeverage : null,
      tradeMode: symbolState ? symbolState.tradeMode : null,
      positionIdx: symbolState ? symbolState.positionIdx : null,
    };
...
/**
 * Valida uma tentativa de setLeverage como redução segura de leverage
 * com conta flat (item 4 da Rodada 6) -- a ÚNICA forma de setLeverage
 * ser autorizado sem ARMED_DEMO. Bloqueia (nunca assume permissivo) em
 * qualquer ambiguidade: valor solicitado ausente/inválido,
 * buyLeverage/sellLeverage divergentes do valor único esperado, acima
 * do teto Demo, snapshot ausente/velho/corrompido/de outra credencial,
 * estado do símbolo desconhecido, posição real aberta (`size` > "0"),
 * qualquer ordem aberta (mesmo condicional/pendente), modo de posição
 * incompatível, leverage efetiva desconhecida, valor solicitado igual
 * (redução desnecessária/ambígua) ou maior (não é redução) que a
 * efetiva. Toda a validação roda em memória, sem I/O de rede -- sempre
 * ANTES de qualquer HMAC/Axios (chamada de dentro de
 * assertDemoOrderAllowedInner, por sua vez dentro de
 * lib/bybit.js::privatePost, antes de authHeaders/axios.post).
 */
function assertSafeLeverageReductionAllowed({ env, params, limits, snapshotPath, now }) {
  const kind = OPERATION_KIND.SAFE_LEVERAGE_REDUCTION;
  const block = (reason) => {
    const err = new DemoOrderBlockedError("DEMO_SAFE_LEVERAGE_REDUCTION_BLOCKED", `Redução segura de leverage bloqueada: ${reason}.`, { reason });
    err.kind = kind;
    throw err;
  };
  ...
  try {
    ...
    return block("leverage_reduction_invalid_value");
  }

  // buyLeverage/sellLeverage precisam vir explícitos e iguais ao valor
  // único solicitado -- lib/bybit.js::setLeverage sempre os monta assim
  // por construção (mesmo `leverage` pros dois lados), mas este gate
  // nunca confia nisso implicitamente (mesma filosofia de nunca confiar
  // em invariante do chamador sem checar -- Rodada 5).
  let buyLeverageStr, sellLeverageStr;
  try {
    buyLeverageStr = decimal.parseStrictDecimal(params.buyLeverage, "buyLeverage");
    sellLeverageStr = decimal.parseStrictDecimal(params.sellLeverage, "sellLeverage");
  } catch {
    return block("leverage_reduction_buy_sell_missing_or_invalid");
  }
  if (decimal.compareDecimalStrings(buyLeverageStr, sellLeverageStr) !== 0 || decimal.compareDecimalStrings(buyLeverageStr, leverageStr) !== 0) {
    return block("leverage_reduction_buy_sell_mismatch");
  }

  if (decimal.compareDecimalStrings(leverageStr, limits.maxLeverage) > 0) {
    return block("leverage_reduction_exceeds_demo_ceiling");
  }

  let snapshot;
  try {
    snapshot = snapshotModule.readTrustedSnapshot({ env, snapshotPath, now });
  } catch (err) {
    throw blockedFromCaughtError(err, kind);
  }

  if (!snapshot.instrumentInfo || snapshot.instrumentInfo.symbol !== params.symbol) {
    return block("leverage_reduction_symbol_mismatch");
  }

  const symbolState = snapshot.symbolState;
  if (!symbolState) {
    return block("leverage_reduction_state_unknown");
  }
  if (symbolState.hasOpenPosition) {
    return block("leverage_reduction_position_open");
  }
  if (!Array.isArray(snapshot.openOrders) || snapshot.openOrders.length > 0) {
    return block("leverage_reduction_open_orders_present");
  }
  if (symbolState.positionIdx !== 0) {
    return block("leverage_reduction_position_mode_incompatible");
  }
  if (symbolState.tradeMode !== EXPECTED_TRADE_MODE) {
    return block("leverage_reduction_trade_mode_unexpected");
  }
  if (symbolState.effectiveLeverage === null || symbolState.effectiveLeverage === undefined) {
    return block("leverage_reduction_current_unknown");
  }

  let effectiveLeverageStr;
  try {
    effectiveLeverageStr = decimal.parseStrictDecimal(symbolState.effectiveLeverage, "effectiveLeverage");
  } catch {
    return block("leverage_reduction_current_unknown");
  }

  const cmp = decimal.compareDecimalStrings(leverageStr, effectiveLeverageStr);
  if (cmp === 0) {
    return block("leverage_reduction_unnecessary");
  }
  if (cmp > 0) {
    return block("leverage_reduction_not_a_reduction");
  }
