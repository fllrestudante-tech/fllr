 ...
 */
function writeLine(filePath, event) {
  ensureDataDir();
...
function log(event) {
  writeLine(config.paths.tradesLog, event);
}

// Trilha separada para transições de saúde/alertas — não mistura com o
// volume alto de eventos de ciclo em trades.jsonl.
function logAlert(event) {
  writeLine(config.paths.alertsLog, event);
}

module.exports = { log, logAlert };
