 ...
 * nenhum momento.
 *
 * order: { symbol, side ("Buy"|"Sell"), qty (string), price (string),
 *   leverage (string), stopLossPrice (string|null), orderLinkId }
 ...
 *   consecutiveErrors, consecutiveLosses, dailyLossPct }
 * now: ms epoch (injetável, nunca Date.now() interno)
 *
 * instrumentInfo: { symbol, qtyStep, minOrderQty, maxOrderQty, tickSize }
 *   -- OBRIGATÓRIO pra qualquer aumento de exposição (item 1 da Rodada
 *   4, nunca mais opcional): ausente/incompleto/de símbolo diferente
 *   bloqueia ANTES de qualquer outro cálculo. lib/demoOrderGate.js é
 *   quem garante que este valor vem do snapshot confiável (autenticado,
 *   fresco, do ambiente Demo) -- nunca de params.instrumentInfo vindo do
 *   chamador de placeOrder, que nem é mais aceito.
 *
 * Arredondamento (item 2 da Rodada 4) -- NUNCA "pro tick mais próximo"
 * genérico:
...
function validateDemoOrder(order, demoState, limits, now, instrumentInfo) {
  ...
  if (!instrumentInfo.qtyStep || !instrumentInfo.minOrderQty || !instrumentInfo.tickSize) {
    return { allowed: false, reason: "instrument_metadata_incomplete" };
  }
  ...
  try {
    qty = decimal.validateInstrumentQty({ qty, qtyStep: instrumentInfo.qtyStep, minOrderQty: instrumentInfo.minOrderQty, maxOrderQty: instrumentInfo.maxOrderQty });
  } catch (err) {
  ...
  return { allowed: true, reason: null, normalized: { qty, price, leverage, stopLossPrice, notionalUsd, projectedExposureUsd } };
}
