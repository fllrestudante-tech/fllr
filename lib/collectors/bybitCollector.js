...
function runCollector(db, eventBus, bybitClient, config, intervals = {}, shouldPause = () => false) {
  ...
  const tickCandles = safe(() => collectCandles(db, eventBus, bybitClient, opts), "candles", metrics, shouldPause);
  const tickFunding = safe(() => collectFunding(db, eventBus, bybitClient, opts), "funding", metrics, shouldPause);
  const tickOi = safe(() => collectOpenInterest(db, eventBus, bybitClient, opts), "open_interest", metrics, shouldPause);
  const tickTicker = safe(() => collectTicker(db, eventBus, bybitClient, opts), "ticker", metrics, shouldPause);
  const tickLongShort = safe(() => collectLongShortRatio(db, eventBus, bybitClient, opts), "long_short_ratio", metrics, shouldPause);
