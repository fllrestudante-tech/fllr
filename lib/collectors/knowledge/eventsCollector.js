...
async function collectFromProvider(db, eventBus, provider, client, metrics, opts = {}) {
 ...
 */
function runCollector(db, eventBus, providers, options = {}) {
  ...
  const timers = providers.map(({ provider, client, intervalMs }) => {
    const tick = () => collectFromProvider(db, eventBus, provider, client, metrics, options);
    tick();
