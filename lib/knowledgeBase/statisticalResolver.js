...
//
// Nota de design (evitar God Object -- ver lib/knowledgeBase/resolver.js):
// este é o "Statistics Resolver" da família de Resolvers especializados.
// Posição revisada no pipeline: Knowledge -> Statistics -> Feature Builder
// (idea-feature-builder, ainda não implementado) -> ESTE Resolver ->
// Signals -> Brain -- o Feature Builder entra ANTES deste Resolver,
// combinando várias `Interpretation` numa Feature nomeada (ex: "Funding
// Extreme" = funding.interpretation.level === EXTREME), pra o Replay
// validar a Feature que o Brain de fato consumiria, não um percentil
// isolado.
const { getAssetStatistics } = require("./assetStatisticsStore");

// Versão de código deste Resolver -- bump manual quando a lógica de
// interpretação mudar (ex: trocar o critério de percentil por MAD).
// Repassada em `evidence.resolverVersion` pra quem consumir (Feature
// Builder) conseguir rastrear "essa Feature foi calculada com qual
// geração do Resolver".
...
function resolveMetricSignal(db, symbol, metric, { scope = "global", windowDays } = {}) {
  ...
  const evidence = {
    ...
    statisticsVersion: stats.window.version,
  };
