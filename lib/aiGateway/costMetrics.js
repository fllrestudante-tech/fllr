...
function estimateCostUsd({ provider, model, promptTokens, completionTokens }) {
  ...
  return (promptTokens / 1e6) * pricing.inputPer1M + (completionTokens / 1e6) * pricing.outputPer1M;
}
 ...
 * provider conhecido (ver nota de topo do arquivo).
 */
function extractUsableUsage(entry) {
  if (!entry.usage || !entry.provider) return null;
  const { promptTokens, completionTokens } = entry.usage;
  if (typeof promptTokens !== "number" || typeof completionTokens !== "number") return null;
  return { provider: entry.provider, model: entry.model, promptTokens, completionTokens };
}
