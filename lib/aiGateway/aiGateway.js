...
const DEFAULT_PROVIDERS = {
  // CRITICO (achado na re-verificacao pos-implementacao, round de auditoria
  // do coordenador): isAgentRouterGated mais abaixo SO controla se o
  // WRAPPER de orcamento e aplicado -- nunca controlou se "agentrouter" e
  // sequer TENTADO. Sem este `config.ai.agentRouterBudgetEnabled &&` aqui,
  // com AI_PROVIDER_ORDER incluindo "agentrouter" (como esta HOJE no .env
  // real deste projeto) e ~/.codex/config.toml presente, hasKey() retornava
  // true e o provider era chamado pelo caminho LEGADO/NAO-GATEADO
  // (effectiveClient = entry.client cru) mesmo com a flag de orcamento
  // desligada -- chamando o processo Codex CLI de verdade, sem orcamento
  // nenhum. Agora, com a flag desligada, hasKey()=false e o loop pula
  // "agentrouter" pelo MESMO mecanismo generico ja usado pra "sem API key
  // configurada" (ver `if (!hasKey) { ...; continue; }` abaixo) -- nunca
  // chega em callProvider, nunca chega em runAgentRouterPrompt, nunca
  // spawna o Codex CLI. Com a flag ligada, comportamento inalterado
  // (isAgentRouterConfigured() decide como sempre).
  agentrouter: { provider: agentrouterProvider, client: agentrouterClientWithConfig, hasKey: () => config.ai.agentRouterBudgetEnabled && isAgentRouterConfigured() },
  ...
  for (const name of order) {
    ...
    try {
      if (isAgentRouterGated) {
        ...
        // Camada fail-closed adicional, independente do policy/ledger já
        // existente: exige AGENTROUTER_DAILY_BUDGET_USD/
        // AGENTROUTER_MONTHLY_BUDGET_USD explícitos no ambiente (nunca um
        // default silencioso) e bloqueia se o mês calendário UTC corrente já
        // esgotou o teto mensal -- ver agentRouterEnvBudget.js. Roda SEMPRE
        // que o gate está ligado, mesmo se um teste sobrescrever `policy`
        // diretamente (gateOverrides.policy), porque é uma checagem
        // ortogonal à política diária em si.
        const {
          resolveEnvBudgetMicrosUsd,
          buildDailyPolicyOptionsFromMicros,
          assertMonthlyBudgetAvailable,
        } = require("./agentRouterEnvBudget");
        const envBudgetOverrides = gateOverrides.envBudget || {};
        const { dailyMicrosUsd, monthlyMicrosUsd } = resolveEnvBudgetMicrosUsd(envBudgetOverrides.env || process.env);
        assertMonthlyBudgetAvailable(gateDbProvider.getDb(), {
          nowMs: envBudgetOverrides.nowFn ? envBudgetOverrides.nowFn() : Date.now(),
          monthlyCapMicrosUsd: monthlyMicrosUsd,
        });

        const policy =
          gateOverrides.policy ||
          require("./agentRouterBudgetPolicy").createAgentRouterBudgetPolicy(buildDailyPolicyOptionsFromMicros(dailyMicrosUsd));
