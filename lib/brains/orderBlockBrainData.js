...
const { buildOrderBlockContext } = require("../knowledgeBase/contextBuilder");
...
function gatherOrderBlockBrainInputs(db, { symbol, interval }) {
  ...
  const { context: orderBlockContext } = buildOrderBlockContext(db, symbol);
  ...
  return {
    ...
    liquidity,
    context,
    confirmAge: orderBlockContext.confirmAge,
    mitigationThreshold: orderBlockContext.mitigationThreshold,
    exhaustionLookback: orderBlockContext.exhaustionLookback,
  };
}
