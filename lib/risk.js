...
function planOrder({ side, price, atr, equity, params, instrumentInfo }) {
  ...
  const roundedQty = instrumentInfo ? roundQtyDown(qty, instrumentInfo.qtyStep) : Number(qty.toFixed(6));
  // Fase D5 -- preço absoluto de cada nível a partir de `r` × stopDistance
  // (mesma unidade de R usada pelo break even/trailing), qty absoluta a
  // partir de qtyPct sobre a quantidade total já arredondada. Nível com
  // `closeRemainder: true` recebe o que sobrou (roundedQty - já alocado) em
  // vez de qtyPct×qty -- garante fechamento total nesse nível mesmo com
  // arredondamento de qtyStep tirando poeira dos níveis anteriores.
  let allocatedQty = 0;
  const tpLevels = (config.tpLevels || []).map((level) => {
    const levelPrice = isBuy ? price + stopDistance * level.r : price - stopDistance * level.r;
    const levelQty = level.closeRemainder
      ? Math.max(0, roundedQty - allocatedQty)
      : instrumentInfo
        ? roundQtyDown(roundedQty * level.qtyPct, instrumentInfo.qtyStep)
        : Number((roundedQty * level.qtyPct).toFixed(6));
    allocatedQty += levelQty;
    return {
      r: level.r,
      price: instrumentInfo ? roundToTick(levelPrice, instrumentInfo.tickSize) : Number(levelPrice.toFixed(4)),
      qty: instrumentInfo ? Number(levelQty.toFixed(decimalsOf(instrumentInfo.qtyStep))) : Number(levelQty.toFixed(6)),
    };
  });
...
module.exports = { canExecute, planOrder, registerTradeResult, registerVolatilityCheck, roundToTick, roundQtyDown };
