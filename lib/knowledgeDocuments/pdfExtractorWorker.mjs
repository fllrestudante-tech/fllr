...
import { reconstructPageText } from "./textReconstruction.js";
  ...
  try {
    ...
    for (const pageNumber of validPages) {
      ...
      try {
        ...
        // Reconstrução geométrica -- roda sobre os itens REAIS de
        // getTextContent() (nunca sobre `rawText`, que já é uma
        // reconstrução mais simples/antiga) -- as duas convivem, nenhuma
        // sobrescreve a outra.
        const reconstruction = reconstructPageText(textContent.items);
        const sourceOrderedTruncated = reconstruction.sourceOrderedText.length > maxOutputCharsPerPage;
        const layoutCandidateTruncated = reconstruction.layoutCandidateText.length > maxOutputCharsPerPage;
        const sourceOrderedText = sourceOrderedTruncated
          ? reconstruction.sourceOrderedText.slice(0, maxOutputCharsPerPage)
          : reconstruction.sourceOrderedText;
        const layoutCandidateText = layoutCandidateTruncated
          ? reconstruction.layoutCandidateText.slice(0, maxOutputCharsPerPage)
          : reconstruction.layoutCandidateText;

        // Truncamento é decidido aqui, pelo chamador (limite de saída),
        // nunca dentro do módulo puro. Cortar sourceOrderedText quebra a
        // igualdade com a concatenação original por construção (o
        // comprimento muda) -- por isso as garantias são RECALCULADAS
        // contra o texto efetivamente emitido (já cortado), nunca
        // herdadas do resultado pré-corte. Isso garante que nenhuma
        // garantia sobrevive a um corte que ela não descreve mais.
        const characterMultisetInvariant = sourceOrderedTruncated ? false : reconstruction.characterMultisetInvariant;
        const sourceOrderInvariant = sourceOrderedTruncated ? false : reconstruction.sourceOrderInvariant;
        const layoutCandidateMultisetInvariant = layoutCandidateTruncated ? false : reconstruction.layoutCandidateMultisetInvariant;

        // truncamento nunca promove qualidade, e texto truncado nunca
        // pode ficar "good" -- se a reconstrução julgou "good" mas o
        // texto foi cortado, rebaixa; se as garantias caíram por causa do
        // corte, força "poor" (mesma regra dura de invariância quebrada).
        let qualityStatus = reconstruction.qualityStatus;
        if (sourceOrderedTruncated) {
          qualityStatus = !characterMultisetInvariant || !sourceOrderInvariant ? "poor" : qualityStatus === "good" ? "review_required" : qualityStatus;
        }
        ...
        pageResults.push({
          ...
          truncated,
          sourceOrderedText,
          sourceOrderedTruncated,
          layoutCandidateText,
          layoutCandidateTruncated,
          reconstructionApplied: reconstruction.reconstructionApplied,
          reconstructionConfidence: sourceOrderedTruncated ? 0 : reconstruction.reconstructionConfidence,
          reconstructionDiagnostics: reconstruction.reconstructionDiagnostics,
          ambiguousGapCount: reconstruction.ambiguousGapCount,
          lineCount: reconstruction.lineCount,
          orientationGroups: reconstruction.orientationGroups,
          layoutCandidateMultisetInvariant,
          layoutCandidateAmbiguousGapCount: reconstruction.layoutCandidateAmbiguousGapCount,
          layoutCandidateLineCount: reconstruction.layoutCandidateLineCount,
          layoutCandidateOrderDiverged: reconstruction.layoutCandidateOrderDiverged,
