...
function simulate(candles, params) {
  ...
  while (i < candles.length) {
    ...
    if (!blockedByCircuitBreaker && (signals[i] === "buy" || signals[i] === "sell")) {
      ...
      const riskUnit = Math.abs(entryPrice - stopPrice); // R -- Fase D3 (break even)
      ...
      for (let j = i + 1; j <= lastIndex; j++) {
        ...
        const hitStop = isBuy ? low <= effectiveStop : high >= effectiveStop;
        ...
        if (hitStop) {
          exitIndex = j;
          outcome = breakEvenActive ? "breakeven" : "stop";
          break;
        }
        if (hitTarget) {
          exitIndex = j;
        ...
        // Break even avaliado DEPOIS da checagem de stop/alvo deste candle --
        // só passa a valer a partir do próximo (mesma convenção pessimista
        // do comentário acima: nunca assume a ordem mais favorável dentro
        // de um candle sem dado intracandle real).
        if (!breakEvenActive) {
      ...
      if (outcome === "target") {
        ...
        tradeReturn = 0; // stop movido pra entrada -- saiu no zero a zero
        breakevens++;
  ...
  const totalTrades = wins + losses + timeouts + breakevens;
  const score = totalReturnPct - 2 * maxDrawdown; // penaliza drawdown pra reduzir overfitting

  return { totalReturnPct, maxDrawdown, wins, losses, timeouts, breakevens, totalTrades, score, tradeReturns };
}
