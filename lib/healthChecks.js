 ...
 * falha, é o estado esperado na maior parte do tempo. Por isso "down" (que
 * carrega conotação de alarme, "deveria estar rodando e não está") é o
 * status errado aqui -- usar "disabled" (nunca rodou / não supervisionado)
 * e "stopped" (já rodou, heartbeat velho, mas não é alarme) evita falso
 * positivo. "down" fica reservado pro arquivo existir e estar corrompido de
 * verdade (isso sim é anômalo, independente de ser manual ou supervisionado).
 */
function checkTelegramRadar(healthFilePath = DEFAULT_TELEGRAM_HEALTH_FILE, now = Date.now()) {
  if (!fs.existsSync(healthFilePath)) {
    return { status: "disabled", details: { reason: "coletor manual, não supervisionado (rode `npm run telegram:watch`)" } };
  }
  try {
