...
/**
 * Lê o heartbeat + métricas que scripts/collector.js escreve periodicamente.
 * degraded quando algum domínio acumula falhas consecutivas -- mesmo com o
 * processo vivo (heartbeat fresco), pode estar falhando silenciosamente
 * contra a Bybit em algum endpoint específico.
 */
function checkCollector(healthFilePath = DEFAULT_COLLECTOR_HEALTH_FILE, now = Date.now()) {
  if (!fs.existsSync(healthFilePath)) {
    return { status: "not_implemented", details: { reason: "scripts/collector.js nunca rodou" } };
  }
  try {
    const raw = JSON.parse(fs.readFileSync(healthFilePath, "utf8"));
    const ageMs = now - new Date(raw.lastHeartbeatAt).getTime();
