...
/**
 * Lê o heartbeat que telegram-radar/watch.js escreve periodicamente. Ao
 * contrário do bot e dos 4 coletores (supervisionados 24/7 por
 * scripts/supervisor.js), o radar hoje é um processo MANUAL -- não está na
 * lista de filhos do supervisor, então não estar rodando agora não é uma
 * falha, é o estado esperado na maior parte do tempo. "MANUAL" (modelo de 7
 * estados, lib/operationalState.js) é o status correto tanto pra "nunca
 * rodou" quanto "já rodou, heartbeat velho" -- nenhum dos dois é alarme.
 * "down" fica reservado pro arquivo existir e estar corrompido de verdade
 * (isso sim é anômalo, independente de ser manual ou supervisionado).
 */
function checkTelegramRadar(healthFilePath = DEFAULT_TELEGRAM_HEALTH_FILE, now = Date.now()) {
  if (!fs.existsSync(healthFilePath)) {
    return { status: "MANUAL", details: { reason: "coletor manual, não supervisionado (rode `npm run telegram:watch`)" } };
  }
  try {
    const raw = JSON.parse(fs.readFileSync(healthFilePath, "utf8"));
    const ageMs = now - new Date(raw.lastHeartbeatAt).getTime();
    if (ageMs > TELEGRAM_STALE_MS) {
      return {
        status: "MANUAL",
        details: { reason: "coletor manual, não supervisionado -- heartbeat antigo indica que não está rodando agora, não necessariamente uma falha", ageMs },
      };
    }
    return { status: "ok", details: { ...raw, ageMs } };
  } catch (err) {
    return { status: "down", details: { error: err.message } };
  }
}
...
const DEFAULT_METRICS_SAMPLER_HEALTH_FILE = path.join(RUNTIME_HEARTBEATS_DIR, "metrics-sampler.json");
const DEFAULT_BACKUP_DAEMON_HEALTH_FILE = path.join(RUNTIME_HEARTBEATS_DIR, "backup-daemon.json");
...
function checkMetricsSampler(healthFilePath = DEFAULT_METRICS_SAMPLER_HEALTH_FILE, now = Date.now()) {
  return checkCollectorHeartbeat(healthFilePath, now);
}
...
function checkBackupDaemon(healthFilePath = DEFAULT_BACKUP_DAEMON_HEALTH_FILE, now = Date.now()) {
  return checkCollectorHeartbeat(healthFilePath, now);
}

/**
 * Distinto de checkBackupDaemon (esse aqui é sobre o PROCESSO estar vivo) --
 * checkBackup lê lib/backup.js::getBackupHealth() e reporta se o backup em
 * si está fresco/válido. Mesma separação já usada em Market Quality
 * (infra vs conteúdo): o daemon pode estar rodando com o processo saudável
 * e ainda assim o último backup ter falhado no integrity_check.
 */
function checkBackup(opts = {}) {
  const health = backup.getBackupHealth(opts);
  const STATUS_MAP = { ok: "ok", never_ran: "not_implemented", stale: "degraded", invalid: "down" };
  return { status: STATUS_MAP[health.status] || "down", details: health };
}
...
/**
 * Lê o snapshot que o Connectivity Manager (scripts/supervisor.js) publica
 * em runtime/connectivity/status.json -- não sonda nada por conta própria,
 * só reporta o que já foi detectado. "down" só enquanto há incidente aberto
 * (online:false); sem snapshot ainda (supervisor não rodou) é not_implemented,
 * não down -- mesma distinção honesta que os outros checks já fazem.
 */
function checkConnectivity(statusFilePath = connectivityStatus.DEFAULT_STATUS_FILE) {
  const status = connectivityStatus.readStatus(statusFilePath);
  if (!status) {
    return { status: "not_implemented", details: { reason: "Connectivity Manager ainda não rodou nenhum tick" } };
  }
  return { status: status.online === false ? "down" : "ok", details: status };
}
...
module.exports = {
  ...
  DEFAULT_METRICS_SAMPLER_HEALTH_FILE,
  DEFAULT_SUPERVISOR_STATE_FILE,
  checkBackupDaemon,
  checkBackup,
  DEFAULT_BACKUP_DAEMON_HEALTH_FILE,
};
