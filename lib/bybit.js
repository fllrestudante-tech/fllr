...
// Classificado como aumento de exposição (conservador -- ver comentário em
// lib/demoOrderGate.js::classifyBybitOperation) -- no perfil demo exige
// ARMED_DEMO, igual a abrir posição. `leverage` vira string decimal pro
...
async function setLeverage(symbol = config.symbol, leverage = config.leverageMax) {
  return privatePost(
    ...
    { category: config.bybit.category, symbol, buyLeverage: String(leverage), sellLeverage: String(leverage) },
    "setLeverage",
...
async function placeOrder({ symbol = config.symbol, side, qty, price, stopLoss, takeProfit, reduceOnly = false, orderLinkId }) {
  ...
  return privatePost("/v5/order/create", body, "placeOrder", { symbol, side, qty: String(qty), price: String(price), stopLoss, reduceOnly, orderLinkId });
}
