...
const { assertTradingExecutionEnabled } = require("./tradingExecutionGate");
...
async function privateGet(path, params = {}) {
  assertTradingExecutionEnabled(); // bloqueia ANTES de qualquer request -- ver lib/tradingExecutionGate.js
...
async function setTradingStop({ symbol = config.symbol, stopLoss, trailingStop, activePrice, tpslMode, takeProfit, tpSize }) {
  ...
  return privatePost("/v5/position/trading-stop", body);
}
