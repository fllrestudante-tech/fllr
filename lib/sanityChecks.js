...
function queryRecentCandleRows(db, windowMs, now = Date.now()) {
  const since = now - windowMs;
  return db.prepare("SELECT open, high, low, close, volume FROM candles WHERE open_time >= ? ORDER BY open_time ASC").all(since);
}
 ...
 */
function sampleSanityChecks(domain, db, { windowMs, now = Date.now() }) {
  ...
  if (domain === "candles") {
    checks.ohlc = checkOhlcValidity(queryRecentCandleRows(db, windowMs, now));
  }
