...
// `symbol` opcional -- mesma regra de lib/dataCoverage.js::countRowsInWindow
// (escopa a 1 símbolo quando informado, agregado de todos quando omitido).
function queryRecentTimestamps(db, table, timeColumn, windowMs, now = Date.now(), symbol) {
  const since = now - windowMs;
  const sql = symbol
    ? `SELECT ${timeColumn} as t FROM ${table} WHERE ${timeColumn} >= ? AND symbol = ? ORDER BY ${timeColumn} ASC`
    : `SELECT ${timeColumn} as t FROM ${table} WHERE ${timeColumn} >= ? ORDER BY ${timeColumn} ASC`;
  const rows = symbol ? db.prepare(sql).all(since, symbol) : db.prepare(sql).all(since);
  return rows.map((r) => r.t);
}

function sampleGaps(domain, db, { windowMs, expectedIntervalMs, toleranceMultiplier = 2, now = Date.now(), symbol }) {
