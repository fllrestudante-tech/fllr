...
//
// Throttle -- runConnectivityCheck() (scripts/supervisor.js) roda a cada
// TICK_INTERVAL_MS=30s pra sempre, o que faria getMe ser chamado ~2880x/dia
// só como health check, sem necessidade real (getMe é essencialmente
// estático -- só muda se o token for revogado ou o Telegram cair). Dentro
// de `throttleMs`, devolve o ÚLTIMO resultado (sucesso OU falha) já
// observado, marcado com throttled:true, em vez de sondar de novo -- o
// resultado é cacheado incondicionalmente (mesmo uma falha), então uma
// recuperação real pode demorar até `throttleMs` a mais pra ser detectada;
// aceitável porque telegram_down é severidade MEDIUM (ver
// SEVERITY_BY_REASON) e afeta só o canal de alerta, nunca trading.
const TELEGRAM_HEALTH_THROTTLE_MS_DEFAULT = 5 * 60 * 1000;
let telegramHealthCache = null; // { result, checkedAtMs }

function checkTelegramHealth({ get, timeoutMs, botToken = config.alerts.telegramBotToken, now = Date.now, throttleMs = TELEGRAM_HEALTH_THROTTLE_MS_DEFAULT } = {}) {
  if (!botToken) return Promise.resolve({ ok: true, error: null, skipped: true });

  const nowMs = now();
  if (telegramHealthCache && nowMs - telegramHealthCache.checkedAtMs < throttleMs) {
    return Promise.resolve({ ...telegramHealthCache.result, throttled: true });
  }
  ...
  return checkProviderHealth({
    ...
    telegramHealthCache = { result, checkedAtMs: nowMs };
    return result;
  });
}

/** Só pra teste -- limpa o cache de throttle entre casos, nunca chamado em produção. */
function __resetTelegramHealthThrottleForTests() {
  telegramHealthCache = null;
}
