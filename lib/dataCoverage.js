...
function countRowsInWindow(db, table, timeColumn, windowMs, now = Date.now()) {
  const since = now - windowMs;
  const row = db.prepare(`SELECT COUNT(*) as c FROM ${table} WHERE ${timeColumn} >= ?`).get(since);
  return row.c;
}
 ...
 */
function sampleCoverage(domain, db, { now = Date.now() } = {}) {
  ...
  const actualCount = countRowsInWindow(db, mapping.table, mapping.timeColumn, windowMs, now);
